...
namespace SNAP
{
    public class PackageChecker
    {
        ...
        private static readonly string PackageKeyword = "Assets/AssetStoreOriginals/_SNAPS_PrototypingAssets/";
                                ...
                                if (package.versions.latestCompatible != "" && package.version != "")
                                    if (GreaterThan(package.versions.latestCompatible, package.version))
                                    {
                                        installRequired[i] = EditorUtility.DisplayDialog("Confirm Package Upgrade",
                                            string.Format(
                                                "The version of \"{0}\" in this project is not the latest version. Would you like to upgrade it to the latest version? (Recommmended)",
                                                packageToAdd[i].name), "Yes", "No");
        ...
        private static bool GreaterThan(string versionA, string versionB)
        {
